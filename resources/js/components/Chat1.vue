    ...
    <v-card class="chat-card" dark="">
      <v-list class="mb-5">
        ...
        <v-list class="p-2" v-for="(item,index) in allMessages" :key="index">
          <v-layout :align-end="(user.id !== item.admin.id)" :class="{'hlw': user.id !== item.admin.id,'hey': user.id === item.admin.id}" column >
              <div class="message-wrapper">
                <v-flex>
                  <span class="small font-italic" :class="{'green--text': user.id !== item.admin.id,'blue--text': user.id === item.admin.id}">{{ item.admin.username }}</span>
                </v-flex>
                <div v-if="item.message" class="text-message-container" style="text-align: justify;">
                  <v-chip :color="(user.id===item.admin_id)?'green':'red' " text-color='white'>
                      {{ item.message }}
                    </v-chip>
                </div>
                ...
                <v-flex class="caption font-italic" :class="{'green--text': user.id !== item.admin.id,'blue--text': user.id === item.admin.id}">
                  {{ item.created_at }}
                </v-flex>
            </div>
